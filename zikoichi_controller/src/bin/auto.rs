...
//const TURTLE_INIT:f64 = 5.544444561004639;
...
async fn main()->Result<(), DynError>
{
    ...
    loop {
        ...
        let x_vec = 0.0;//t_odom.pose.pose.position.x - (imu_odom.pose.pose.position.x + TURTLE_INIT);
        let y_vec = 0.0;//t_odom.pose.pose.position.y - (imu_odom.pose.pose.position.y + TURTLE_INIT);
        let ro_vec = t_yaw - imu_yaw;

        pr_info!(log, "x:{}, y:{}", x_vec, y_vec);
